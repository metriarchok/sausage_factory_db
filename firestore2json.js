      ...
      return db
        ...
        .then(data => {
          ...
          data.forEach(doc => {
            ...
            promises.push(
              firestore2json(db.collection(collection).doc(doc.id), schema[collection], current[collection][doc.id])
            );
          });
...
firestore2json(admin.firestore(), { ...schema }, {}).then(res =>
  fs.writeFileSync(`./backups/local_db_${Math.floor(Date.now() / 1000)}.json`, JSON.stringify(res, null, 2), 'utf8')
);
